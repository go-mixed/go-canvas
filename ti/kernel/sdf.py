...
@ti.kernel
def compute_distance_field_euclidean(
...
@ti.kernel
def compute_distance_field_manhattan(
    mask: ti.types.ndarray(dtype=ti.f32, ndim=2),
    dist: ti.types.ndarray(dtype=ti.f32, ndim=2),
    max_dist: ti.f32
):
    """
    计算二值遮罩的距离场（使用曼哈顿距离近似）
    ...
    for i in range(w):
        for j in range(h):
            if dist[i, j] > 0.5:
                d = ti.cast(max_dist, ti.f32)
                if i > 0:
                    d = ti.min(d, dist[i - 1, j] + 1.0)
                if j > 0:
                    d = ti.min(d, dist[i, j - 1] + 1.0)
                dist[i, j] = d
